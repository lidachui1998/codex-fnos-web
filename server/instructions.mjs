...
- 当你自主启动子代理时，主任务负责完整收口：持续跟踪自己启动的全部直接及嵌套子代理，等待它们进入 completed、shutdown、failed 或 interrupted 等终态，汇总结果并关闭不再需要的代理后，才能向用户发送最终答复。除非用户明确要求停止或不再等待，不得在仍有子代理运行、等待批准或等待输入时提前结束主任务；新的用户消息也不得自动唤醒与本轮无关的历史子代理。
