  ...
  return <Modal open={open} title="定时任务" subtitle="由飞牛工作台在后台触发；应用服务停止或 NAS 关机时不会运行。" onClose={onClose} wide>
    ...
    {importing ? <AutomationImportPanel projects={projects} onCancel={resetForm} onImported={(task, replacedExisting) => { const prefix = replacedExisting ? "已更新" : "已导入"; setNotice(blockingIssues(task).length > 0 ? `${prefix}电脑任务“${task.name}”并保持暂停；下一步请点“兼容试运行”。` : `${prefix}电脑任务“${task.name}”并已按原状态启用。`); setImporting(false); void load(); }} /> : !editing ? <>
      ...
      <div className="form-grid two">
        ...
        }}><option value="follow">跟随所属项目默认</option><option value="openai">OpenAI / ChatGPT</option>{form.providerMode === "provider" && !bootstrap.providers.some((provider) => provider.id === form.providerId && provider.enabled) && <option value={`provider:${form.providerId}`}>原供应商已移除（请重新选择）</option>}{bootstrap.providers.filter((provider) => provider.enabled).map((provider) => <option key={provider.id} value={`provider:${provider.id}`}>{provider.name}</option>)}</select></label>
