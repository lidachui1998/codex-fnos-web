          ...
          {tab === "appearance" && (
            <div className="settings-section compact-settings">
              ...
              {bootstrap.appearance.hasBackground && <>
                ...
                <div className="setting-actions"><button className="secondary-button compact" onClick={() => void perform("background-clear", () => api("/api/settings", { method: "PATCH", body: JSON.stringify({ backgroundOpacity: 0.8, backgroundBlur: 0, backgroundPanelOpacity: 0.5 }) }), "已切换为清晰背景显示")}>清晰显示</button></div>
                <label className="range-row"><span>图片可见度 <em>{Math.round(bootstrap.settings.backgroundOpacity * 100)}%</em></span><input type="range" min="0.2" max="1" step="0.05" value={bootstrap.settings.backgroundOpacity} onChange={(event) => void perform("background-opacity", () => api("/api/settings", { method: "PATCH", body: JSON.stringify({ backgroundOpacity: Number(event.target.value) }) }), "图片可见度已更新")} /></label>
                <label className="range-row"><span>背景柔化 <em>{bootstrap.settings.backgroundBlur}px</em></span><input type="range" min="0" max="8" step="1" value={bootstrap.settings.backgroundBlur} onChange={(event) => void perform("background-blur", () => api("/api/settings", { method: "PATCH", body: JSON.stringify({ backgroundBlur: Number(event.target.value) }) }), "背景柔化已更新")} /></label>
                <label className="range-row"><span>内容面板遮罩 <em>{Math.round(bootstrap.settings.backgroundPanelOpacity * 100)}%</em></span><input type="range" min="0.2" max="0.9" step="0.05" value={bootstrap.settings.backgroundPanelOpacity} onChange={(event) => void perform("background-panel", () => api("/api/settings", { method: "PATCH", body: JSON.stringify({ backgroundPanelOpacity: Number(event.target.value) }) }), "内容面板遮罩已更新")} /></label>
              </>}
            </div>
          )}
